...
def _worksheet_xml(cols_widths, rows, freeze_row=None, merges=None):
    parts = [
        ...
        for rec in cp_records:
            ...
            for p in rec["patterns"]:
                w.writerow([
                    rec["cpId"],
                    rec["cpName"],
                    "null" if p.get("uriPattern") is None else p.get("uriPattern"),
                    "null" if p.get("minAuthenticationLevel") is None else p.get("minAuthenticationLevel"),
                    "null" if p.get("exposeExternally") is None else p.get("exposeExternally"),
                ])
...
def build_workbook(csi_id, cp_records):
    ...
    for rec in cp_records:
        ...
        if rec.get("error"):
            rows1.append((r_idx, [
                (1, cp_id, STYLE_DATA_MERGED),
                (2, cp_name, STYLE_DATA_MERGED),
        ...
        start_row = r_idx
        for i, p in enumerate(rec["patterns"]):
            ...
            if i == 0:
                rows1.append((r_idx, [
                    ...
                    (2, cp_name, STYLE_DATA_MERGED),
                    (3, _disp(uri), style),
                    (4, _disp(min_auth), style),
                    (5, _disp(expose), style),
                ]))
            else:
                rows1.append((r_idx, [
                    (3, _disp(uri), style),
        ...
        if end_row > start_row:
            merges.append(f"A{start_row}:A{end_row}")
            merges.append(f"B{start_row}:B{end_row}")
    ...
    return [
        {"name": "Patterns",  "cols": [12, 32, 70, 20, 20], "rows": rows1, "freeze_row": header_row + 1, "merges": merges},
